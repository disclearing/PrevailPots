    ...
    private void showList(final int pageNumber, final String label, final CommandSender sender) {
        ...
        for(final Player target : Bukkit.getOnlinePlayers()) {
            if(senderPlayer == null || senderPlayer.canSee(target)) {
                final PlayerFaction playerFaction = this.plugin.getFactionManager().getPlayerFaction(target.getUniqueId());
        ...
        for(final Map.Entry<PlayerFaction, Integer> entry : sortedMap) {
            ...
            builder.append(" (" + ChatColor.GREEN+ entry.getValue() + '/' +playerFaction2.getMembers().size() + ") "+ChatColor.GRAY + "[DTR " + playerFaction2.getDtrColour() + JavaUtils.format(playerFaction2.getDeathsUntilRaidable()) + ChatColor.GRAY + '/' + ChatColor.GREEN + JavaUtils.format(playerFaction2.getMaximumDeathsUntilRaidable())+ ChatColor.GRAY + "]", ComponentBuilder.FormatRetention.FORMATTING).color(net.md_5.bungee.api.ChatColor.GREEN);
        ...
        sender.sendMessage(ChatColor.GOLD + " Faction List " + ChatColor.GREEN + "[" + pageNumber + '/' + maxPages + ']');

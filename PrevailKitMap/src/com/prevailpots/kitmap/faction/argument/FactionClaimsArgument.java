...
public class FactionClaimsArgument extends CommandArgument {
    ...
    public FactionClaimsArgument(final HCF plugin) {
        super("claims", "Visualize all the claims for an faction. (Displayed by Pillars)");
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(final CommandSender sender, final Command command, final String label, final String[] args) {
        final PlayerFaction selfFaction = (sender instanceof Player) ? this.plugin.getFactionManager().getPlayerFaction(((Player) sender).getUniqueId()) : null;
        ...
        sender.sendMessage(ChatColor.YELLOW + "Claims of " + ChatColor.GOLD + targetFaction.getDisplayName(sender));
        for(final Claim claim : claims) {
            sender.sendMessage(ChatColor.YELLOW + " " + claim.getFormattedName());
        }
        return true;
    }

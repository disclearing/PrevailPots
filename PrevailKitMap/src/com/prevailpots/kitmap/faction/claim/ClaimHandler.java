package com.prevailpots.kitmap.faction.claim;
...
import com.prevailpots.kitmap.faction.type.WildernessFaction;
...
public class ClaimHandler {
    ...
    public boolean canClaimHere(final Player player, final Location location) {
        ...
        if(!(this.plugin.getFactionManager().getFactionAt(location) instanceof WildernessFaction)) {
            player.sendMessage(ChatColor.RED + "You can only claim land in the " + ConfigurationService.WILDERNESS_COLOUR + "Wilderness" + ChatColor.RED + ". " + "Make sure you are past " + plugin.getHcfHandler().getWarZoneRadius() + " blocks from spawn.");
            return false;
        }
        final PlayerFaction playerFaction = this.plugin.getFactionManager().getPlayerFaction(player.getUniqueId());
        if(playerFaction == null) {
        ...
        for(int x = minimumX; x < maximumX; ++x) {
            for(int z = minimumZ; z < maximumZ; ++z) {
                ...
                if(factionAt != null && !(factionAt instanceof WildernessFaction)) {
                    player.sendMessage(ChatColor.RED + "This claim contains a location not within the " + ConfigurationService.WILDERNESS_COLOUR + factionAt.getDisplayName(player) + ChatColor.RED + '.');
